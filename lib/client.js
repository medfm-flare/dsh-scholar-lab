...
//                               -> ctx.remote.scholarLab.search / .exportCsv
//     (the host half provides the `scholarLab` service; see lib/host.js)
//   - React is required from the module table instead of arriving as a global.
window.__ModuleLoader__.load({
  id: '@yourscope/dsh-scholar-lab',
  factory: (require) => {
    var module = { exports: {} };
    ...
    const inject = ['slots', 'remote', 'remote.scholarLab']
...
function apply(ctx) {
    ...
    function useExport(defaults) {
      ...
      const doExport = function (query, results) {
        ...
        ctx.remote.scholarLab.exportCsv({ query: query, results: results, withDetails: withDetails })
          .then(function (res) {
    ...
    function ScholarLabPanel(props) {
      ...
      const run = function () {
        ...
        setError('')
        setState(null)
        ctx.remote.scholarLab.search({ query: q, maxResults: 10, start: 0 })
          .then(function (res) {
      ...
      const loadMore = function () {
        ...
        setMoreBusy(true)
        setError('')
        ctx.remote.scholarLab.search({ query: state.query, maxResults: 10, start: state.results.length })
          .then(function (res) {
