...
export function apply(ctx, config) {
    ...
    // ---- HTTP bridge for the browser half ----------------------------------
    // The client bundle is a community plugin: dsh's remote.* RPC namespaces
    // exist only for first-party typert descriptors, so browser-to-host calls
    // go over plain same-origin webServer routes (the dshmarket pattern).
    ...
    function readJsonBody(request, limit) {
      return new Promise(function (resolve, reject) {
        ...
        request.on('end', function () {
          try { resolve(JSON.parse(Buffer.concat(chunks).toString('utf8') || '{}')) } catch (e) { reject(new Error('invalid JSON body')) }
        })
    ...
    // Browser half reaches these over plain same-origin HTTP (community
    // plugins have no remote.* typert namespace). Same body logic as the
    // scholarLab service above, minus the RPC transport.
    ctx.inject(['webServer'], (hostCtx) => {
      hostCtx.effect(() => {
        const disposers = [
          hostCtx.webServer.register({
            kind: 'exact',
            path: '/scholar-lab/search',
            handler: async function (request, response) {
              ...
              try {
                ...
                if (!query) {
                  sendJson(response, 400, { error: 'query is required', query: '', count: 0, results: [] })
                  return
                }
                const maxResults = Math.min(Math.max(parseInt(body.maxResults, 10) || 10, 1), 20)
                const start = Math.max(parseInt(body.start, 10) || 0, 0)
                const v = await runSearch(query, maxResults, start, '', 'en')
                sendJson(response, v.error ? 502 : 200, v)
              } catch (e) {
                sendJson(response, 500, { error: String((e && e.message) || e), query: '', count: 0, results: [] })
              }
            }
          }),
          hostCtx.webServer.register({
            kind: 'exact',
            path: '/scholar-lab/export',
            handler: async function (request, response) {
              if (request.method !== 'POST') {
                response.writeHead(405, { allow: 'POST' })
                response.end()
              ...
              try {
                ...
                if (!query || results.length === 0) {
                  sendJson(response, 400, { error: 'no results to export', csv: '', filename: '', rows: 0 })
                  return
                }
                const enriched = await enrichBatch(results, !!body.withDetails)
                const csv = buildCsv(query, enriched)
                sendJson(response, 200, {
                  error: '',
                  csv: csv,
                  filename: 'scholar-export-' + slugOf(query) + '-' + stamp() + '.csv',
                  rows: enriched.length
                })
              } catch (e) {
                sendJson(response, 500, { error: String((e && e.message) || e), csv: '', filename: '', rows: 0 })
              }
            }
          })
        ]
        return function () { for (let i = 0; i < disposers.length; i++) disposers[i]() }
      }, 'scholar-lab: http routes')
    })
  }
