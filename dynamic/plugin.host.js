return {
  apply(ctx) {
    ...
    // journal-impact-factors.json next to the dsh working directory (optionally
    // scholar-lab/journal-impact-factors.json). When the file is absent the
    // plugin degrades gracefully (no IF / no full-name expansion).
    ...
    const searchTool = harness.defineTool({
      ...
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Search query, e.g. "attention is all you need" or "transformer time series forecasting"' },
          ...
          export_csv: { type: 'boolean', description: 'When true, also write the results to a CSV file in the workspace (scholar-lab/scholar-export-*.csv) and return its path in csv_path', default: false }
        },
        required: ['query']
      },
      output: {
    ...
    const abstractTool = harness.defineTool({
      ...
      parameters: {
        type: 'object',
        properties: {
          url: { type: 'string', description: 'Paper URL, e.g. the link field of a scholar_search result' }
        },
        required: ['url']
      },
      output: {
    ...
    ctx.effect(function () {
      const d1 = harness.handle('scholar-search', async function (args) {
        ...
        try {
          return await runSearch(query, maxResults, 0, '', 'en')
        } catch (e) {
          return { error: String((e && e.message) || e), query: query, count: 0, results: [] }
        }
      })
      const d2 = harness.handle('scholar-export', async function (args) {
        const a = (args && typeof args === 'object') ? args : {}
        const query = String(a.query || '').trim()
        const results = Array.isArray(a.results) ? a.results : []
        const withDetails = !!a.withDetails
        if (!query || results.length === 0) return { error: 'no results to export', path: '', rows: 0 }
        try {
          const enriched = await enrichBatch(results, withDetails)
          const csv = buildCsv(query, enriched)
          const path = await writeCsvFile(csv, slugOf(query))
          return { path: path, rows: enriched.length, error: '' }
        } catch (e) {
          return { error: String((e && e.message) || e), path: '', rows: 0 }
        }
      })
      const d3 = harness.registerTool(ctx, searchTool)
      const d4 = harness.registerTool(ctx, abstractTool)
